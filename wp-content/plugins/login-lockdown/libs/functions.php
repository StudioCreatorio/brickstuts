...
class LoginLockdown_Functions extends LoginLockdown
{
    ...
    static function check_2fa()
    {
        ...
        $user_roles_2fa = $options['2fa_roles'];

        $user = wp_get_current_user();
        $user_has_2fa_role = false;

        foreach($user_roles_2fa as $role){
            if ( in_array( $role, (array) $user->roles ) ) {
                $user_has_2fa_role = true;
            }
        }

        if(false === $user_has_2fa_role){
            return;
        }
        ...
        $login_message_text = 'Check your email to confirm the log in';
        if(!empty($options['2fa_login_text'])){
            $login_message_text = $options['2fa_login_text'];
        }

        $login_link_text    = 'Log out and log in again to resend the email';
        if(!empty($options['2fa_link_text'])){
            $login_link_text = $options['2fa_link_text'];
        }
        ...
        if (!empty($_GET['loginlockdown_2fa_confirm'])) {
            ...
            if (sanitize_text_field(@$_GET['key_hash']) != md5('elephant' . sanitize_text_field(@$_GET['key']))) {
                wp_die('Invalid 2FA hash value.', '2FA Authentication', array('link_text' => $login_link_text, 'link_url' => wp_logout_url()));
            }
            if (sanitize_text_field(@$_GET['key']) != get_user_meta($user->ID, 'loginlockdown_2fa_key', true)) {
                wp_die('Invalid 2FA key.', '2FA Authentication', array('link_text' => $login_link_text, 'link_url' => wp_logout_url()));
            }
        ...
        if (empty(get_user_meta($user->ID, 'loginlockdown_2fa_key', true))) {
            wp_die('No 2FA key found.', '2FA Authentication', array('link_text' => $login_link_text, 'link_url' => wp_logout_url()));
        }
        ...
        if (empty(get_user_meta($user->ID, 'loginlockdown_2fa_key', true)) || get_user_meta($user->ID, 'loginlockdown_2fa_key', true) != get_user_meta($user->ID, 'loginlockdown_2fa_key_check', true)) {
            wp_die($login_message_text, '2FA Authentication', array('link_text' => $login_link_text, 'link_url' => wp_logout_url()));
        }
    }
    ...
    static function wp_login($user_login, $user)
    {
        ...
        $link_2fa = trailingslashit(get_bloginfo('url')) . '?loginlockdown_2fa_confirm=1&key=' . $key . '&key_hash=' . md5('elephant' . $key) . '&redirect_to=' . @$_POST['redirect_to'] . '&rand=' . rand(100, 999);

        if(!empty($options['2fa_email_message'])){
            $message = $options['2fa_email_message'];
            $message = str_replace('%%user%%', $user_login, $message);
            $message = str_replace('%%user_display_name%%', $user->display_name, $message);
            $message = str_replace('%%site_title%%', get_bloginfo('name'), $message);
            $message = str_replace('%%site_url%%', get_bloginfo('url'), $message);
            $message = str_replace('%%login_ip%%', LoginLockdown_Utility::getUserIP(), $message);
            $message = str_replace('%%login_ua%%', $user_login, $message);
            $message = str_replace('%%login_time%%', date('Y-m-d H:i:s', time()), $message);

            if(strpos($message, '%%confirm_link%%') !== false){
                $message = str_replace('%%confirm_link%%', $link_2fa, $message);
            } else {
                $message .= PHP_EOL . PHP_EOL . trailingslashit(get_bloginfo('url')) . '?loginlockdown_2fa_confirm=1&key=' . $key . '&key_hash=' . md5('elephant' . $key) . '&redirect_to=' . @$_POST['redirect_to'] . '&rand=' . rand(100, 999);
            }
        } else {
            $message = 'Please click the link below in order to confirm log in to ' . get_bloginfo('name') . PHP_EOL;
            $message .= 'If you did not login recently somebody might have your username and password. Change the password immediately.' . PHP_EOL;
            $message .= 'Request originated from: ' . LoginLockdown_Utility::getUserIP() . ' for user ' . $user->user_login . PHP_EOL . PHP_EOL;
            $message .= trailingslashit(get_bloginfo('url')) . '?loginlockdown_2fa_confirm=1&key=' . $key . '&key_hash=' . md5('elephant' . $key) . '&redirect_to=' . @$_POST['redirect_to'] . '&rand=' . rand(100, 999);
        }

        if(!empty($options['2fa_email_message'])){
            $subject = $options['2fa_email_subject'];
            $subject = str_replace('%%user%%', $user_login, $subject);
            ...
            $subject = str_replace('%%login_ip%%', LoginLockdown_Utility::getUserIP(), $subject);
            $subject = str_replace('%%login_ua%%', $_SERVER['HTTP_USER_AGENT'], $subject);
            $subject = str_replace('%%login_time%%', date('Y-m-d H:i:s', time()), $subject);
        } else {
            $subject = '2FA login confirmation for ' . get_bloginfo('name');
        }
        ...
        wp_mail($user->user_email, $subject, nl2br($message));
    ...
    static function login_head(){
        ...
        if ($options['design_enable']) {
            ...
            if (!empty($options['design_logo'])) {
                ...
                if($background_width != 'auto'){
                    $background_width = $background_width . 'px';
                }

                if($background_height != 'auto'){
                    $background_height = $background_height . 'px';
                }

                echo 'width: ' . $background_width . ';';
                echo 'height: ' . $background_height . ';';
                echo 'background-size:' . $background_width . ' ' . $background_height . ';';
    ...
    public static function login_cookie_expiration($expire)
    {
        ...
        if ((int)$options['cookie_lifetime'] > 14) {
            return (int)$options['cookie_lifetime'] * DAY_IN_SECONDS;
        }
    ...
    static function wp_loaded()
    {
        ...
        if (!(isset($_GET['action']) && $_GET['action'] === 'postpass' && isset($_POST['post_password']))) {
            ...
            if(is_user_logged_in() && substr($request['path'], strlen(self::trailingslashit($options['login_url']))*-1, strlen(self::trailingslashit($options['login_url']))) == self::trailingslashit($options['login_url'])){
    ...
    static public function login_url_welcome_email($value)
    {
        $options = LoginLockdown_Setup::get_options();
    ...
    static function logout_url($logout_url, $redirect)
    {
        ...
        $logout_url = str_replace('wp-login.php', $options['login_url'], $logout_url);
    ...
    static public function site_status_tests($tests)
    {
