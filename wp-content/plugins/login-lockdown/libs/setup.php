...
class LoginLockdown_Setup extends LoginLockdown
{
    ...
    static function load_actions()
    {
        ...
        add_action('wf_licensing_lockdown_remote_action_disable_2fa', function ($request) {
            $options = self::get_options();
            $options['2fa_email'] = 0;
            update_option(LOGINLOCKDOWN_OPTIONS_KEY, $options);
            wp_send_json_success(array('result' => 'disabled'));
        }, 10, 1);
    ...
    static function default_options()
    {
        $defaults = array(
            ...
            '2fa_email_subject'                       => '',
            '2fa_email_message'                       => '',
            '2fa_login_text'                          => 'Check your email to confirm the log in',
            '2fa_link_text'                           => 'Log out and log in again to resend the email',
            'whitelabel'                              => 0,
            'captcha'                                 => 'disabled',
            'captcha_secret_key'                      => '',
            'captcha_site_key'                        => '',
