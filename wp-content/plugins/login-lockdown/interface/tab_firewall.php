...
class LoginLockdown_Tab_Firewall extends LoginLockdown
{
    ...
    static function tab_general()
    {
        ...
        echo '<br /><span>Directory traversal (also known as file path traversal) is a web security vulnerability that allows an attacker to access files on the server that they should not by passing file paths that attempt to traverse the normal directory structure using the parent folder path.</span>';
        echo '</td></tr>';
    ...
    static function tab_2fa()
    {
        ...
        echo '<br /><span>After the correct username &amp; password are entered the user will receive an email with a one-time link to confirm the login.<br>In case somebody steals the username &amp; password they still won\'t be able to login without access to the account email.<br /><br /> If you do not receive your 2FA email and get locked out you can disable 2FA remotely from your <a href="https://dashboard.wploginlockdown.com/sites/" target="_blank">Login Lockdown Dashboard</a> under the website\'s Actions menu</span>';
        echo '</td></tr>';


        echo '<tr valign="top" id="2fa_roles-roles">
        <th scope="row">2FA User Roles</th>
        <td>';

        foreach ($roles as $tmp_role) {
            echo  '<input name="' . LOGINLOCKDOWN_OPTIONS_KEY . '[2fa_roles][]" id="roles-' . $tmp_role['val'] . '" ' . LoginLockdown_Utility::checked($tmp_role['val'], $options['2fa_roles'], false) . ' value="' . $tmp_role['val'] . '" type="checkbox" /> <label for="roles-' . $tmp_role['val'] . '">' . $tmp_role['label'] . '</label><br />';
        }
        ...
        echo '<tr valign="top">
        <th scope="row"><label for="2fa_login_text">2FA Login Message</label></th>
        <td><input type="text" class="regular-text" id="2fa_login_text" name="' . LOGINLOCKDOWN_OPTIONS_KEY . '[2fa_login_text]" value="' . $options['2fa_login_text'] . '" />';
        echo '<br /><span>Message users see after login instructing them to check their email.</span>';
        echo '</td></tr>';

        echo '<tr valign="top">
        <th scope="row"><label for="2fa_link_text">2FA Login Link Text</label></th>
        <td><input type="text" class="regular-text" id="2fa_link_text" name="' . LOGINLOCKDOWN_OPTIONS_KEY . '[2fa_link_text]" value="' . $options['2fa_link_text'] . '" />';
        echo '<br /><span>Text of the resend 2fa email link.</span>';
        echo '</td></tr>';

        echo '<tr valign="top">
        <th scope="row"><label for="2fa_email_subject">2FA Email Subject</label></th>
        <td><input type="text" class="regular-text" id="2fa_email_subject" name="' . LOGINLOCKDOWN_OPTIONS_KEY . '[2fa_email_subject]" value="' . $options['2fa_email_subject'] . '" />';
        echo '<br /><span>Subject of 2FA email. Leave blank to use default subject. You can use any of the variables below.</span>';
        echo '</td></tr>';

        echo '<tr valign="top">
        <th scope="row"><label for="2fa_email_message">2FA Email Content</label></th>
        <td><textarea id="2fa_email_message" name="' . LOGINLOCKDOWN_OPTIONS_KEY . '[2fa_email_message]" rows="8" style="width:100%;">';
        echo $options['2fa_email_message'];
        echo '</textarea>';
        echo '<br /><span>You can use this field to create a custom confirmation message for the 2FA email. If blank the default message will be used. Below you have a list of variables that you can use in the message:
            <ul>
            <li><strong>Confirmation Link: </strong>%%confirm_link%% <u>(if you do not use this variable anywhere in the message, the confirmation link will be added to the bottom of the email)</u></li>
            <li><strong>User: </strong>%%user%%</li>
            <li><strong>User Display Name: </strong>%%user_display_name%%</li>
            <li><strong>Site Title: </strong>%%site_title%%</li>
            <li><strong>Site URL: </strong>%%site_url%%</li>
            <li><strong>Login IP: </strong>%%login_ip%%</li>
            <li><strong>Login User Agent: </strong>%%login_ua%%</li>
